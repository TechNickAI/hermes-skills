...
| `learn-from-conversations` | Turn recurring requests and repeated corrections into new skills or targeted updates       | `HERMES_HOME` read access                                    |
...
| `prove-the-setting-works`  | Prove a config change actually does what you're about to promise, on the real code path    | —                                                            |
| `check-upstream-first`     | Find the running version and the existing upstream fix before you patch anything yourself  | —                                                            |
| `is-it-really-broken`      | A check said BROKEN — verify that before you repeat it, because a timeout means unknown    | —                                                            |
| `delegation-handoff`       | Brief delegated work so it lands, and verify the artifact instead of trusting the report   | —                                                            |
